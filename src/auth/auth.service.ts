...
export class AuthService {
  ...
  async login(loginDto: LoginDto) {
    ...
    const token = this.generateToken(user.id, user.email, user.roleId, user.companyId);
  ...
  private generateToken(userId: number, email: string, roleId: number, companyId:number): string {
